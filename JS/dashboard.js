...
function initializeEventListeners() {
    ...
    setupMembersModalEventListeners();
}
...
function setupMembersModalEventListeners() {
    // Búsqueda
...
async function cargarMiembrosModal() {
    try {
        ...
        mostrarMiembros();
        actualizarPaginacion();
        document.getElementById('totalAccounts').textContent = cuentasTotales;
    } catch (err) {
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <div class="text-xs text-gray-400">UID: ${member.UID || member.id}</div>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
...
function mostrarModalMembresias() {
    ...
    getDocs(collection(db, 'subscripciones')).then(snap => {
        snap.forEach(doc => {
            const d = doc.data();
            if (d.nombre && d.costo) {
...
window.generarReporteUsuarios = generarReporteUsuarios;
window.generarReporteAsistencias = generarReporteAsistencias;
window.generarReporteMembresias = generarReporteMembresias;
window.generarReporteIngresos = generarReporteIngresos;